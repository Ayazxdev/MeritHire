...
class DualLLMClient:
    ...
    def __init__(self, openrouter_api_key: Optional[str] = None):
        ...
        # User requested Claude 3.5 Sonnet for detection
        self.cloud_model = "anthropic/claude-3-haiku" 
    ...
    def call_openrouter(self, prompt: str, system_prompt: str = "") -> Dict:
        ...
        print(f"\n[DualLLMClient] 🌐 Preparing OpenRouter Call...", file=sys.stderr)
        print(f"[DualLLMClient] Model: {self.cloud_model}", file=sys.stderr)
        
        if not self.openrouter_api_key:
            print("[DualLLMClient] ❌ ERROR: API Key Missing!", file=sys.stderr)
            return {"content": "", "success": False, "error": "No API Key"}
        
        print(f"[DualLLMClient] 🔑 API Key loaded (Ends with: ...{self.openrouter_api_key[-4:]})", file=sys.stderr)
        ...
        try:
            ...
            print("[DualLLMClient] 🚀 Sending Request...", file=sys.stderr)
            response = requests.post(self.openrouter_url, json=payload, headers=headers, timeout=60)
            print(f"[DualLLMClient] 📥 Response Status: {response.status_code}", file=sys.stderr)
            
            if response.status_code == 200:
                data = response.json()
                content = data["choices"][0]["message"]["content"]
                print(f"[DualLLMClient] ✅ Success! Content Length: {len(content)}", file=sys.stderr)
                return {"content": content, "success": True, "model": "openrouter"}
            else:
                logger.error(f"OpenRouter Error {response.status_code}: {response.text}")
                print(f"[DualLLMClient] ❌ FAILED: {response.status_code} - {response.text[:100]}", file=sys.stderr)
                return {"content": "", "success": False, "error": f"Status {response.status_code}"}
                
        except Exception as e:
            logger.error(f"OpenRouter Failed: {e}")
            print(f"[DualLLMClient] 💥 EXCEPT: {str(e)}", file=sys.stderr)
            return {"content": "", "success": False, "error": str(e)}

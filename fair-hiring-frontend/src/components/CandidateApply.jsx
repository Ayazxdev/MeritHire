        ...
        >
            ...
            <header className="sticky top-0 left-0 w-full bg-[#E6E6E3] border-b-[3px] border-[#1c1c1c] z-[100] px-6 md:px-12 py-6 flex justify-between items-center bg-opacity-95 backdrop-blur-sm">
                ...
                <div className="font-grotesk text-[10px] font-black tracking-widest uppercase opacity-40">
                    ID: {String(roleId || '').slice(0, 8) || 'REF-882'}
                </div>
            </header>
                        ...
                        >
                            <div className="w-full max-w-4xl space-y-20">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                    {['ATS', 'GITHUB', 'LEETCODE', 'CODEFORCES', 'LINKEDIN', 'SKILL', 'TEST', 'BIAS', 'MATCHING', 'PASSPORT'].map((s, i) => {
                                ...
                                {pipelineStatus?.credential_preview && (
                                    <motion.div
                                        initial={{ opacity: 0, y: 10 }}
                                        animate={{ opacity: 1, y: 0 }}
                                        className="p-8 border-4 border-black bg-white shadow-[8px_8px_0px_#000]"
                                    >
                                        <label className="font-grotesk text-[10px] font-black uppercase tracking-widest block mb-4 border-b border-black/10 pb-2">LIVE DATA EXTRACTION</label>
                                        <div className="flex flex-wrap gap-4">
                                            {pipelineStatus.credential_preview.skills?.slice(0, 5).map(skill => (
                                                <span key={skill} className="px-3 py-1 bg-black text-white font-grotesk text-[9px] font-black uppercase">{skill}</span>
                                            ))}
                                            {pipelineStatus.credential_preview.github_verified && <span className="px-3 py-1 border border-black font-grotesk text-[9px] font-black uppercase">GH_VERIFIED</span>}
                                        </div>
                                    </motion.div>
                                )}
                            </div>
